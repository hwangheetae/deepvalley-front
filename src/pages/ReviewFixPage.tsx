...
import { updateReview } from '../api/ReviewUpdateApi/ReviewUpdateApi';
...
const ReviewFixPage: React.FC = () => {
  const { review, reviewId } = useLoaderData() as {
  ...
  const [visitedDate, setVisitedDate] = useState<Date | null>(
    new Date(review.visited_date),
  );
  const [privacy, setPrivacy] = useState(review.privacy);
  const [tags, setTags] = useState<string[]>(review.tag_names);
